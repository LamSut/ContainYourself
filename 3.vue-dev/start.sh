...
docker build -t vue-dev:v1 . || { echo "Failed to build Docker image"; exit 1; }

# deploy with Minikube
echo "Applying Kubernetes deployment..."
minikube kubectl -- apply -f vue-dev-deployment.yaml || { echo "Failed to apply Kubernetes deployment"; exit 1; }
...
minikube kubectl -- port-forward svc/vue-dev-service 5173:5173 &
